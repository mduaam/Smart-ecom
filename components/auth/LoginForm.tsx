...
export default function LoginForm({ locale }: { locale: string }) {
    ...
    // Wrapper to inject locale and handle state
    const handleLogin = async (prevState: any, formData: FormData) => {
        formData.append('locale', locale);
        return login(formData); // Returns { error: string } or redirects
    };
            ...
            <form action={formAction} className="space-y-6">
                ...
                {state?.error && (
                    <div className="p-4 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/50 flex items-center gap-3 text-red-600 dark:text-red-400 text-sm">
                        ...
                        <p>{t(state.error === 'Invalid login credentials' ? 'error.invalidCredentials' : 'error.serverError')}</p>
                    </div>
                )}
